...
class TestThread < Test::Unit::TestCase
  ...
  def test_source_container
    ...
    cont = 'file'
    eval '1.times{cont = RubyVM::Frame.get.source_container[0]}'
  ...
  def test_thread_tracing
      skip("Don't have tracing?  in 2.1.5")
    assert_equal(false, Thread.get.tracing?)
    Thread.get.tracing = true
    assert_equal(true, Thread.get.tracing?)
    Thread.get.tracing = false
    assert_equal(false, Thread.get.tracing?)
  end

  def test_thread_exec_event_tracing
      skip("Don't have exec_event_tracing in 2.1.5")
      assert_equal(false, Thread.current.exec_event_tracing?)
      Thread.get.exec_event_tracing = true
      assert_equal(true, Thread.current.exec_event_tracing?)
      Thread.get.exec_event_tracing = false
      assert_equal(false, Thread.current.exec_event_tracing?)
  end
  ...
  def test_fields(notused=nil)
      ...
      puts "Reinstate arity and argc test"
      ...
      assert_equal('run_test', tf_prev.method)
      ...
      1.times do
          ...
          assert_equal(['file', 'test-frame.rb'], tup[0...-1])
      ...
      1.upto(1) do
          # We could use ".prev" below instead of '.get.prev", but we
          # may as well test get.prev.
          tf = RubyVM::Frame::get.prev
