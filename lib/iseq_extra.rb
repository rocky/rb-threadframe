...
class RubyVM::InstructionSequence
  ...
  def locate_line(line)
    offsetlines.each_pair do |ip, val|
      return ip if val.member?(line)
    end
  end

  # iseq and instruction address (IP) of the first instruction on
  # the specified line. This method recursively examines child
  # compiled methods until an exact match for the searched line is
  # found.  It returns both the matching CompiledMethod and the IP
  # of the first instruction on the requested line, or nil if no
  ...
  def locate_line_with_children(line)
    iseq = self
    ip = iseq.locate_line(line)
    return iseq, ip if ip
    ...
    child_iseqs.each do |child_iseq|
      next if child_iseq.equal? iseq
      if res = locate_line_with_children(line, child_iseq)
        return res
      end
    end
